...
Insert into schools (school_name,location,prinacipal_name,since, number_of_students,numer_facultys,play_ground,library,computer_lab)values
('Sarswathi vidya savamuste','Rajajinagara','Lokesh',1998,1850,160,'YES' ,'YES','YES');

Create table Students_from(reg_no int not null,First_name varchar(20) unique,Last_name varchar(20),Age int not null,
Date_of_birth date not null,id int,Blood_group varchar(5) not null ,hight varchar(5) not null,
weight varchar(5) not null,result varchar(20) not null,foreign key(ID) references schools(id));
